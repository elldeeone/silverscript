...
boolean D_Display(void)
{
    ...
    static int kaspa_display_log_count;
    ...
    redrawsbar = false;
    if (kaspa_display_log_count < 8) {
        printf("kaspa-doom:display-stage:start gamestate=%d gametic=%d wipegamestate=%d\n",
               gamestate, gametic, wipegamestate);
    }
    ...
    if (kaspa_display_log_count < 8) {
        printf("kaspa-doom:display-stage:before-update-no-blit\n");
    }
    I_UpdateNoBlit();
    if (kaspa_display_log_count < 8) {
        printf("kaspa-doom:display-stage:after-update-no-blit\n");
    }
    ...
    if (kaspa_display_log_count < 8) {
        printf("kaspa-doom:display-stage:before-menu\n");
    }
    M_Drawer();  // menu is drawn even on top of everything
    if (kaspa_display_log_count < 8) {
        printf("kaspa-doom:display-stage:before-netupdate\n");
    }
    NetUpdate(); // send out any new accumulation
    if (kaspa_display_log_count < 8) {
        printf("kaspa-doom:display-stage:done wipe=%d\n", wipe);
        ++kaspa_display_log_count;
    }
...
//  D_RunFrame
//
EMSCRIPTEN_KEEPALIVE void D_RunFrame()
{
    int nowtime;
    int tics;
    static int wipestart;
    static boolean wipe;
    static int kaspa_frame_log_count;

    if (kaspa_frame_log_count < 8) {
        printf("kaspa-doom:frame:%d gamestate=%d menu=%d paused=%d wipe=%d\n",
               kaspa_frame_log_count + 1, gamestate, menuactive, paused, wipe);
        ++kaspa_frame_log_count;
    }
    ...
    if (kaspa_frame_log_count <= 8) {
        printf("kaspa-doom:frame-stage:before-tics\n");
    }
    TryRunTics(); // will run at least one tic
    if (kaspa_frame_log_count <= 8) {
        printf("kaspa-doom:frame-stage:after-tics gametic=%d\n", gametic);
    }

    S_UpdateSounds(players[consoleplayer].mo); // move positional sounds
    if (kaspa_frame_log_count <= 8) {
        printf("kaspa-doom:frame-stage:after-sound\n");
    }
    ...
    if (screenvisible && !nodrawers) {
        ...
        if (wipe) {
            if (kaspa_frame_log_count <= 8) {
                printf("kaspa-doom:frame-stage:display-wipe\n");
            }
            ...
            I_FinishUpdate(); // page flip or blit buffer
            if (kaspa_frame_log_count <= 8) {
                printf("kaspa-doom:frame-stage:after-display\n");
            }
        }
    }
}
...
void D_DoomLoop(void)
{
    ...
    emscripten_set_main_loop(D_RunFrame, TICRATE, 1);
}
