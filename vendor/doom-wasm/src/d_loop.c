...
void TryRunTics(void)
{
    ...
    static int kaspa_try_log_count;
    int realtics;
    int availabletics;
    int counts;

    if (kaspa_try_log_count < 16) {
        printf("kaspa-doom:try-stage:start gametic=%d maketic=%d\n", gametic, maketic);
    }
    ...
    if (singletics) {
        ...
        if (kaspa_try_log_count < 16) {
            printf("kaspa-doom:try-stage:before-netupdate\n");
        }
        NetUpdate();
        if (kaspa_try_log_count < 16) {
            printf("kaspa-doom:try-stage:after-netupdate maketic=%d\n", maketic);
        }
    }
    ...
    if (kaspa_try_log_count < 16) {
        printf("kaspa-doom:try-stage:counts lowtic=%d available=%d real=%d\n",
               lowtic, availabletics, realtics);
    }
    ...
    while (!PlayersInGame() || lowtic < gametic / ticdup + counts) {
        if (kaspa_try_log_count < 16) {
            printf("kaspa-doom:try-stage:wait players=%d lowtic=%d target=%d counts=%d\n",
                   PlayersInGame(), lowtic, gametic / ticdup + counts, counts);
            ++kaspa_try_log_count;
        }
        NetUpdate();
    ...
    while (counts--) {
        ...
        if (kaspa_try_log_count < 16) {
            printf("kaspa-doom:try-stage:before-runtic gametic=%d counts=%d\n", gametic, counts + 1);
        }
        ...
        for (i = 0; i < ticdup; i++) {
            ...
            if (kaspa_try_log_count < 16) {
                printf("kaspa-doom:try-stage:after-runtic gametic=%d\n", gametic);
            }
            gametic++;
            KaspaStateBridge_AfterTic(gametic, set->cmds, set->ingame, NET_MAXPLAYERS);
            if (kaspa_try_log_count < 16) {
                printf("kaspa-doom:try-stage:after-bridge gametic=%d\n", gametic);
                ++kaspa_try_log_count;
            }
